...
_See a detailed list of all the flags in gmx_MMPBSA command
line [here](../../Documentation/command-line#calling-gmx_mmpbsa-from-the-command-line)_
...
ligand is DNA) amber format topologies and trajectories will be obtained from that of the complex. To do so, a MD
Structure+mass(db) file (`com.tpr`), an index file (`index.ndx`), a trajectory file (`com_traj.xtc`), and both the
receptor and ligand group numbers in the index file (`1 12`) are needed. The `mmpbsa.in` input file will contain all the
parameters needed for the MM/PB(GB)SA calculation. In this case, 16
frames `(endframe-startframe)/interval = (21-5)/1 = 16`
...
Also a high dielectric constant `intdiel=10` will be used because of the high number of charged residues at the
interface.
...
* the complex structure file format
    ...
    {:.bg-red-light}
    The * .gro format does not contain information related to chains.

* GROMACS version 
  
    {:.bg-red-light}
    We have seen that the GROMACS 20xx.x versions, `trjconv` omit the chain IDs.
  
* options used to generate the topology in GROMACS (referring to the `-merge` option)

    {:.bg-red-light}
    If you use the -merge option then GROMACS will merge the chains into one.
...
```note
Currently, the notation for selecting the residue to mutate is CHAIN:RES_NUM. We are evaluating whether it is 
convenient to change the format to CHAIN:RES_NAME:RES_NUM. This notation allows you to double check
```
